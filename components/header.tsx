...
export default function Header() {
    ...
    <header className='z-[999] relative'>
        ...
        <nav className="fixed flex top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
            <ul className='flex w-[22rem] items-center justify-center flex-wrap gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-2'>
                {links.map( link => (
                    <motion.li className='h-3/4 flex items-center justify-center'
                    key={link.name}
                    initial={{ y: -100, opacity: 0}}
                    animate={{ y: 0, opacity: 1 }}>
                        <Link className={clsx("relative flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition", {
                        ...
                        >
                            ...
                            {(activeSection === link.name) && (
                                <motion.span className="absolute inset-1 rounded-full bg-gray-200/40 -z-10"
